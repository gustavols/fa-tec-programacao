...
class ListagemConsumoEmValor extends Listagem {
  ...
  public listar(): void {
    ...
    console.log(
      "Listagem de 10 clientes que mais consumiram produtos/serviços em valor"
    );
    console.log(
      "- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -"
    );
    ...
    this.clientes.forEach((cliente) => {
      ...
      let quantidadeTotalConsumida: number = 0;
        ...
        this.servicos.forEach((servicosCadastrados) => {
          if (servicosConsumidos == servicosCadastrados.getId) {
            quantidadeTotalConsumida =
              quantidadeTotalConsumida + servicosCadastrados.preco;
          }
        });
      });
    ...
    let consumoClientesLimitado: any[] = consumoClientesOrdenado.slice(0, 4);
    ...
    consumoClientesLimitado.forEach((consumoClientes) => {
      this.clientes.forEach((cliente) => {
        if (consumoClientes.cpf === cliente.getCpf.getValor) {
          console.log(
            "Valor total de serviços/produtos consumidos pelo cliente: R$" +
...
export default ListagemConsumoEmValor;
